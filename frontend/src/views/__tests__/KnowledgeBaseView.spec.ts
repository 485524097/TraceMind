...
vi.mock('@/services/knowledgeBases', () => ({ listKnowledgeBases: vi.fn(), deleteKnowledgeBase: vi.fn(), createKnowledgeBase: vi.fn(), updateKnowledgeBase: vi.fn() }))
const mockedList = vi.mocked(listKnowledgeBases)
const mockedDelete = vi.mocked(deleteKnowledgeBase)
const kb: KnowledgeBase = { id: '8eaa2608', name: 'Backend Notes', description: 'Architecture records', created_at: '2026-07-17T01:00:00Z', updated_at: '2026-07-17T02:00:00Z' }

function resp(items: KnowledgeBase[]): KnowledgeBaseListResponse { return { items, total: items.length, offset: 0, limit: 100 } }

function mountView() {
  return mount(KnowledgeBaseView, {
    global: { stubs: { RouterLink: { template: '<a><slot /></a>' }, KnowledgeBaseFormDialog: true,
      ElDropdown: { props: ['trigger'], template: '<div class="el-dropdown"><slot /><slot name="dropdown" /></div>' },
      ElDropdownMenu: { template: '<div class="el-dropdown-menu"><slot /></div>' },
      ElDropdownItem: { props: ['disabled'], template: '<button class="el-dropdown-item" :disabled="disabled" :data-testid="$attrs[\'data-testid\']"><slot /></button>' } } },
  })
}

describe('KnowledgeBaseView', () => {
  beforeEach(() => {
    vi.restoreAllMocks(); mockedList.mockReset(); mockedDelete.mockReset()
  ...
  it('shows empty state', async () => {
    ...
    const wrapper = mountView(); await flushPromises()
    expect(wrapper.text()).toContain('No knowledge bases')
  })
  ...
  it('renders dropdown with contextual actions per KB', async () => {
    ...
    const editBtn = wrapper.findAll('.el-dropdown-item').find(b => b.text() === 'Edit')
    const delBtn = wrapper.get('[data-testid="delete-8eaa2608"]')
    expect(editBtn).toBeTruthy()
    expect(delBtn.text()).toBe('Delete')
  })
