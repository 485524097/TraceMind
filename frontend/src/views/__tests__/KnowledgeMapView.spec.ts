...
describe('KnowledgeMapView', () => {
  ...
  it('shows an empty-state prompt and a safe load error', async () => {
    ...
    await flushPromises()
    expect(emptyWrapper.text()).toContain('Save a conversation answer as knowledge')
    ...
    await flushPromises()
    expect(errorWrapper.get('[role="alert"]').text()).toBe('Knowledge Map could not be loaded.')
  })
})
