...
Dense 索引配置使用 `QDRANT_COLLECTION_NAME`、`QDRANT_DENSE_VECTOR_NAME`、`EMBEDDING_MODEL_NAME`、`EMBEDDING_DIMENSION`、`EMBEDDING_BATCH_SIZE`、`EMBEDDING_DEVICE` 和 `DOCUMENT_INDEX_STALE_AFTER_SECONDS`。backend 与 celery-worker 必须保持一致。首次实际调用 SentenceTransformer 会下载模型；CI 单元测试使用 FakeEmbeddingProvider，不下载模型。
