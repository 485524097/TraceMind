...
Qdrant Cloud。当前不实现 Reranker、Weighted RRF 或检索评测集自动调参。
...
`POST /api/v1/knowledge-bases/{knowledge_base_id}/search/hybrid`。RAG 默认使用
Hybrid Retrieval。Sparse 召回可能改变 no-answer 行为，仍需用真实资料持续验收。
