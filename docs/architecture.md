...
- Celery：异步执行 DocumentVersion 解析；每个任务创建独立 AsyncEngine/Session 并在结束时释放。
...
当前尚未实现 OCR、AST 解析、全文或向量索引、Reranker、RAG、模型供应商和用户权限。“文件已导入”不代表“文件已解析”，“解析成功”也不代表“已建立检索索引”。DocumentChunk 尚未写入 Qdrant。
