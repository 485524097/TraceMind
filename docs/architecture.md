...
- Reranker 层：主 Backend 只使用 HTTP Provider；CrossEncoder 在独立单 Worker FastAPI 进程中单例加载。
...
RAG 和 Reranked 调试 API 先获取 Hybrid Top 10，再把文档名、可选章节和原始 Chunk
正文发送给仅监听本机的 Reranker 服务。CrossEncoder 返回 candidate ID、rank 和原始
Logit，应用层据此重排原对象并保留 RRF score/rank 与全部引用元数据。

Reranker Client 在主 Backend lifespan 中单例创建，但主 Backend 启动不依赖模型
readiness。RAG 对连接失败、超时、503、OOM 和无效响应回退 Hybrid Top 5；
`/search/reranked` 不静默降级。CrossEncoder 不进入主 Backend、Celery、数据库、
Redis、Qdrant 或 LLM 生命周期。
...
当前尚未实现 OCR、AST 解析、Reranker 训练/微调、Weighted RRF、检索评测集、对话历史、Agent 和用户权限。
