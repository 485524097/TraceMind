...
- Qdrant：保存单一 Collection 中的 Dense named vector 与可追溯 Payload。
...
解析成功后单独派发 indexing task。Worker 以 generation UUID claim 版本，在数据库事务外批量生成 Embedding、写入 Qdrant 并核对 Point 数；最终通过行锁和 generation compare-and-set 激活索引，再尽力清理旧 generation。语义检索先从 PostgreSQL 获取当前版本的 active generation，再使用这些 generation 过滤 Qdrant。

## Citation-grounded RAG 数据流

RAG API 复用 Dense Search 和 active generation，Context Builder 将完整原始 Chunk 与
...
当前尚未实现 OCR、AST 解析、BM25、Sparse/Hybrid Search、Reranker、对话历史、Agent 和用户权限。“文件已导入”不代表“文件已解析”，“解析成功”也不代表“已建立检索索引”。
